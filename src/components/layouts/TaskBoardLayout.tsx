...
import type { TaskEffortType } from '../../types/db-types/taskEffortType';
              ...
              <AccordionPanel key={list.id}>
                <div className="relative group">
                    ...
                    <div className="w-full flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <span>{list.title || list.name || VOCAB.UNTITLED_LIST}</span>
                            ...
                            <div className="flex gap-1">
                              {listGroups[list.id].map((listGroup: any) => (
                                <Tooltip
                                  ...
                                  placement="top"
                                >
                                  <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800 hover:bg-blue-200 cursor-help">
                  ...
                  <div className="absolute top-1/2 right-12 transform -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-50">
                    <CrudAction
                      listTitle={list.title || list.name || VOCAB.UNTITLED_LIST}
                      identificator={list.id}
                      lable="Name"
                      textValue={list.title || list.name || VOCAB.UNTITLED_LIST}
                      brainDumpType="NEW_LIST"
                    />
                  </div>
                </div>
                <AccordionContent>
...
{VOCAB.NO_TASKS_IN_LIST}
        ...
        <div>{VOCAB.NO_TASK_LISTS_FOUND}</div>
      )}
      <CreateTaskModal open={!!createForListId} onClose={() => setCreateForListId(null)} listId={createForListId || ''} isRoutineMode={false} />
    </div>
  );
} 